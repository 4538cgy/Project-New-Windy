...
class BillActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.activityBillButtonOrderComplete.setOnClickListener { startActivity(Intent(binding.root.context,OrderCompleteActivity::class.java))
        finish()}
        ...
        binding.activityBillRecycler.layoutManager = LinearLayoutManager(binding.root.context,LinearLayoutManager.VERTICAL,false)

        binding.activityBillEdittextAddress.setOnClickListener { startActivityForResult(Intent(binding.root.context, SearchAddressActivity::class.java),100)}
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                100 -> {
                    binding.activityBillEdittextAddress.setText(data!!.getStringExtra("address_arg1")
                        .toString() +
                ...
                .collection("product").document(it).get().addOnSuccessListener {
                    if(it != null) {
                        if (it.exists()) {
                            ...
                            binding.activityBillTextviewTotalCost.text = "최종 결제 금액 : " + cost().toString() + "원"
    ...
    inner class BillProductRecyclerViewAdapter() : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            val binding = ItemBillProductListBinding.inflate(LayoutInflater.from(binding.root.context),parent,false)
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            holder.binding.itemBillProductListTextviewCost.text = productList[position].cost.toString()
    ...
    class BillProductRecyclerViewHolder(val binding : ItemBillProductListBinding) : RecyclerView.ViewHolder(binding.root){
